...
test('dashboard displays statistics for the current organization', function () {
    ...
    $this->actingAs($user)
        ...
        ->assertSeeInOrder(['P/Lサマリー', 'B/Sサマリー', '固定資産', '予実管理', '管理'])
        ...
        ->assertDontSee('id="accounting-study-summary"', false)
        ->assertDontSee('id="quick-actions"', false)
...
test('company administrators see the ordered global navigation menu', function () {
    ...
    $this->actingAs($user)
        ...
        ->assertOk()
        ->assertSeeInOrder(['ダッシュボード', '仕訳', '固定資産', '予実管理', '会計学習', '管理'])
...
test('general users do not see the global navigation management menu', function () {
    ...
    $this->actingAs($user)
        ->get(route('profile.show'))
        ->assertOk()
        ->assertSeeInOrder(['ダッシュボード', '仕訳', '固定資産', '予実管理', '会計学習'])
        ->assertSee('href="'.route('accounting-study.index').'"', false)
