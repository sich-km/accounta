<form method="POST" action="{{ $action }}" class="space-y-6">
    ...
    <div>
        <x-label for="code" value="勘定科目コード" />
        <x-input
            id="code"
            name="code"
            type="text"
    ...
    <div>
        ...
        <select id="account_type" name="account_type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
            ...
            @foreach ($accountTypes as $value => $label)
                <option value="{{ $value }}" @selected(old('account_type', $account?->account_type) === $value)>
                    {{ $label }}
                </option>
            @endforeach
        </select>
    ...
    <div class="flex items-center justify-end gap-3">
        <a href="{{ route('accounts.index') }}" class="text-sm text-gray-600 underline hover:text-gray-900">
            キャンセル
        </a>
